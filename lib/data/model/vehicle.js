...
async function get(params) {
    if (params.ve_reg_num) {
        if (Array.isArray(params.ve_reg_num)) { 
            expression = jsonata("vehicle[ve_reg_num in $ve_reg_num]");
            multipleResult = true;
        }
        else {
        ...
        const ve_reg_num = (await proof.get({ ve_proof_id: params.ve_proof_id})).ve_reg_num
        expression = jsonata("vehicle[ve_reg_num = $ve_reg_num][0]");
        bindings = { ve_reg_num: ve_reg_num };
        multipleResult = false;
    ...
    } else if (params.ve_auto_renew === true) {
        expression = jsonata("vehicle[ve_auto_renew = true]");
        bindings = {};
        multipleResult = true;
    ...
    // Get roadtax rate
    if (result && params.get_roadtax_rate === true) {
        const _params = {
        ...
        result.forEach((vehicle, i, result) => {
            let data;
            if (vehicle.ve_is_electric === true) {
            ...
            roadtaxRate = roadtaxClass.base_rate + (roadtaxClass.progressive_rate * ((vehicle.ve_engine_capacity - (roadtaxClass.capacity_lower_limit - 1)) / (vehicle.ve_is_electric === true ? 50 : 1)) );
            
            vehicle.ve_roadtax_rate = Math.round((roadtaxRate + Number.EPSILON) * 100) / 100;
            result[i] = vehicle;
        });
    }

    if (result && params.get_proof_id) { // get_proof_id == u_id
        proofs = await proof.get({
            u_id: params.get_proof_id,
            ve_reg_num: vehicles,
        });

        result.forEach(vehicle => {
            vehicleProof = proofs[vehicle.ve_reg_num] ?? null;
            if (vehicleProof == null) return;
            vehicle.ve_proof_id = vehicleProof.ve_proof_id;
        });
    }
